...
get_validate_template_CASES = [
    ...
    {"id": "file_not_present", "args": ["--file", "test.yaml"],
     "expected_output": ["Failed to find", r"test\.yaml template file\."]},
    {"id": "path_as_file", "args": ["--file", "."],
     "expected_output": ["Failed to find", r" template file\."]},
    ...
    {"id": "no_templates_found", "args": [],
     "expected_output": r"No Templates found\."},
    ...
    {"id": "local_template_id", "add_template": True, "args": ["--id", "template-local"],
     "expected_output": r"- template-local \(local\)"},
    ...
    {"id": "global_template_id", "add_template": True, "args": ["--id", "template-global"],
     "expected_output": r"- template-global \(global\)"},
    ...
    {"id": "local_template_not_found_command", "add_template": True, "args": [],
     ...
     "expected_output": [
         r"template-global\.yaml: is not valid",
         r"Failed to find call command file ~[\/|\\].prich[\/|\\]templates[\/|\\]template-global[\/|\\]scripts[\/|\\]echo1",
         r"Failed to find call command file ~[\/|\\].prich[\/|\\]templates[\/|\\]template-global[\/|\\]scripts[\/|\\]echo2",
         "template-local.yaml: is not valid",
         r"Failed to find call command file \.[\/|\\].prich[\/|\\]templates[\/|\\]template-local[\/|\\]scripts[\/|\\]echo1",
         r"Failed to find call command file \.[\/|\\].prich[\/|\\]templates[\/|\\]template-local[\/|\\]scripts[\/|\\]echo2",
     ]},
    {"id": "local_template_not_found_python", "add_template": True, "args": [],
     ...
     "expected_output": [
         r"template-global\.yaml: is not valid",
         r"Failed to find isolated venv at ~[\/|\\]\.prich[\/|\\]templates[\/|\\]template-global[\/|\\]scripts",
         r"Failed to find call python file ~[\/|\\].prich[\/|\\]templates[\/|\\]template-global[\/|\\]scripts[\/|\\]echo1.py",
         r"Failed to find call python file ~[\/|\\].prich[\/|\\]templates[\/|\\]template-global[\/|\\]scripts[\/|\\]echo2.py",
         r"template-local\.yaml: is not valid",
         r"Failed to find isolated venv at \.[\/|\\].prich[\/|\\]templates[\/|\\]template-local[\/|\\]scripts",
         r"Failed to find call python file \.[\/|\\].prich[\/|\\]templates[\/|\\]template-local[\/|\\]scripts[\/|\\]echo1.py",
         r"Failed to find call python file \.[\/|\\].prich[\/|\\]templates[\/|\\]template-local[\/|\\]scripts[\/|\\]echo2.py",
     ]},

    # wrong templates from resources
    {"id": "file_empty",
     "args": ["--file", "{resources}/empty.yaml"],
     "expected_output": [r"empty\.yaml: is not valid", "check if file or contents are correct"]},
    {"id": "file_no_schema_version",
     "args": ["--file", "{resources}/no_schema_version.yaml"],
     "expected_output": [r"no_schema_version\.yaml", "is not valid", "Failed to load template",
                         "Unsupported template schema version NOT SET"]},
    {"id": "file_not_supported_schema",
     "args": ["--file", "{resources}/not_supported_schema.yaml"],
     "expected_output": [r"not_supported_schema\.yaml", "is not valid", "Failed to load template",
                         r"Unsupported template schema version 0\.1"]},
    {"id": "file_no_id",
     "args": ["--file", "{resources}/no_id.yaml"],
     "expected_output": [r"no_id\.yaml", r"is not valid \(1 issue\)", "Failed to load template",
                         "Missing required field at 'id'", r"\+id: \.\.\."]},
    {"id": "file_no_name",
     "args": ["--file", "{resources}/no_name.yaml"],
     "expected_output": [r"no_name\.yaml", "is not valid", "Failed to load template",
                         "Missing required field at 'name'", r"\+name: \.\.\."]},
    {"id": "file_no_steps",
     "args": ["--file", "{resources}/no_steps.yaml"],
     "expected_output": [r"no_steps\.yaml", "is not valid", "Failed to load template",
    ...
    {"id": "file_wrong_steps",
     ...
     "expected_output": [r"wrong_steps\.yaml", r"is not valid \(3 issues\)", "Failed to load template",
                         r"1\. Field value 'provider' found using 'type' does not match any of the expected values: 'python', 'command', 'llm', 'render' at 'steps\[1\]':  ---  - name: Ask to generate 1st",
                         r"2\. Missing required field at 'steps\[2\]\.name':  ---  step_name: Ask to generate 2nd",
                         r"3\. Unrecognized field at 'steps\[2\].step_name':  ---  step_name: Ask to generate 2nd",
                         "See Steps documentation:", "https://oleks-dev.github.io/prich/reference/template/steps/"]},
    {"id": "file_wrong_variables",
     "args": ["--file", "{resources}/wrong_variables.yaml"],
     "expected_output": [r"wrong_variables\.yaml", r"is not valid \(4 issues\)", "Failed to load template",
                         r"1\. Missing required field at 'variables\[1\]\.name':  ---  var_name: test  \+name: \.\.\.",
                         r"2\. Unrecognized field at 'variables\[1\]\.var_name':  ---  var_name: test  \.\.\. ",
                         r"3\. Field value should be 'str',",
                         r"4\. Field value should be a valid boolean, unable to interpret input at 'variables\[4\]\.required'",
    ...
    {"id": "file_no_python_not_found_isolated_venv",
     ...
     "expected_output": [r"no_venv\.yaml", r"is not valid \(1 issue\)",
                         r"1\. Failed to find isolated venv at",
                         "There are no steps with type 'python' found",
                         r"Install it by running 'prich venv-install test-template'\."
     ]},
    {"id": "file_no_python_not_found_shared_venv",
     "args": ["--file", "{resources}/no_shared_venv.yaml"],
     "expected_output": [r"no_shared_venv\.yaml", r"is not valid \(1 issue\)",
                         r"1\. Failed to find shared venv at",
                         "There are no steps with type 'python' found",
     ]},
    {"id": "file_yaml_error",
     "args": ["--file", "{resources}/yaml_error.yaml"],
     "expected_output": [r"yaml_error\.yaml", r"is not valid \(1 issue\)",
                         "Failed to load template:", r"1\. while scanning a simple key",
    ...
    with runner.isolated_filesystem(temp_dir=mock_paths.home_dir):
        ...
        if case.get("expected_output") is not None:
            ...
            for expected_output in case.get("expected_output"):
                assert re.search(expected_output, result.output.replace("\n", " ").replace("  ", " "))
