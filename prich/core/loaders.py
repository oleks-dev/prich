...
def load_local_config() -> Tuple[Optional[ConfigModel], Path]:
    return load_config_model(Path.cwd() / ".prich/config.yaml")

def load_global_config() -> Tuple[Optional[ConfigModel], Path]:
...
def load_merged_config() -> Tuple[ConfigModel, list[Path]]:
    ...
    if local_config and local_config.providers:
        result = local_config, [local_path]
    if global_config and global_config.providers:
...
def _load_template_model(yaml_file: Path, base_dir: Path = None) -> TemplateModel | None:
    ...
    try:
        ...
        raise click.ClickException(f"Failed to load template {template.get('name') if template else '?'} from {yaml_file}: {', '.join([x.get('msg') for x in e.errors()])}")
...
def load_template_models(global_only: bool = False) -> list[TemplateModel]:
...
# def find_template(template_name: str, global_only: bool = False) -> Tuple[Optional[dict], str]:
#     def _find(base_dir: Path):
#         folder = base_dir / ".prich/templates" / template_name
#         yaml_file = folder / f"{template_name}.yaml"
#         if yaml_file.is_file():
#             return _load_yaml(yaml_file), str(folder)
#         return None, ""
#
#     if not global_only:
#         template, folder = _find(Path.cwd())
#         if template:
#             template["source"] = "local"
#             template["folder"] = folder
#             return template, "local"
#
#     template, folder = _find(Path.home())
#     if template:
...
def get_loaded_templates(tags=None):
    if tags:
